...
A detailed runbook, part of the [exapp-operations](../SKILL.md) skill. Read this when the deploy daemon is
Kubernetes; for the common Docker path, concepts, and the full command references, stay in
[operations.md](operations.md). Kubernetes support is **NC34+**.
...
> app_api's `tests-deploy-k8s*.yml` workflows, and HaRP's `development/redeploy_host_k8s.sh`; HaRP's README
