...
A detailed runbook, part of the [exapp-operations](../SKILL.md) skill. Read this when ExApp containers should
run on a **different machine** than Nextcloud, for example a GPU server for AI ExApps. For the single-host
golden path, concepts, and command references, stay in [operations.md](operations.md).
...
**Step 1. Run HaRP next to Nextcloud** exactly as in the `operations.md` Quickstart (Steps 2-4: shared key,
...
Run HaRP on the remote machine exactly as in the `operations.md` Quickstart Step 3 (it mounts that host's
...
- **Re-registering after a fix changes nothing**: `daemon:register` is a no-op for an existing name;
  `daemon:unregister` first (see `operations.md` Troubleshooting).

For daemon flags and general lifecycle, see [operations.md](operations.md).
